...
def apply_growth(
    ...
    if rate_type == "cagr":
        rates = {
            grp: calc_cagr(start_vals[grp], end_vals[grp], periods)
            for grp in list(end_vals.index)
        }
    else:  # linear AAGR
        rates = {
            grp: calc_aagr(start_vals[grp], end_vals[grp], periods)
            for grp in list(end_vals.index)
        }

    # Build projection years (end_year..proj_to)
    years = list(range(end_year, proj_to + 1))
    exp = pd.MultiIndex.from_product(
        [end_vals.index, years], names=[group_col, year_col]
...
def forward_fill(
    df, value_col="value", country_col="iso3_code", year_col="year", limit=2
):
    ...
    for key, g in d.groupby(country_col, sort=False):
        ...
        gg = g.set_index(year_col).reindex(
            years
        )  # may introduce completely missing years
    ...
    out[value_col] = out[f"{value_col}"].where(
        out[f"{value_col}"].notna(), out[f"{value_col}_filled"]
    )
...
def chart_1():
    ...
    df = (
        GHED_DATA.loc[
            lambda d: (d.indicator_code.isin(["che_usd2022"])) & (d.year != 2023)
        ]
        # forward fill missing values up to 2 years
        .pipe(
            forward_fill,
            value_col="value",
            country_col="iso3_code",
            year_col="year",
            limit=2,
        )
        .groupby(["year"])
    ...
    (
        df.assign(units="Constant 2022 US$")
    ...
    # export chart data
    (
        df.rename(columns={"value": "actual_spending"})
        ...
        .assign(
            annotate=lambda d: np.where(d.variable == "actual_spending", True, None)
        )
        ...
        .assign(
            value_annotation=lambda d: round(
                (d.actual_spending / 1000000000000), 2
            ).astype(str)
            + " trillion"
        )
        .reset_index()
...
def chart_2():
    ...
    df = (
        GHED_DATA.loc[lambda d: d.indicator_code.isin(["che_usd2022"])]
    ...
    (
        ...
        .rename(columns={"value": "actual_spending"})
        .melt(
            id_vars=["year", "income_level"],
            value_vars=["actual_spending", "expected_spending"],
        )
        # assign an annotate column with values of True where "variable" is "actual spending"
        .assign(
            annotate=lambda d: np.where(d.variable == "actual_spending", True, None)
        ...
        .pivot(
            index=["year", "annotate", "income_level"],
            columns="variable",
            values="value",
        )
        .reset_index()
        .assign(
            value_annotation=lambda d: round(
                (d.actual_spending / 1000000000), 2
            ).astype(str)
            + " billion"
        )
