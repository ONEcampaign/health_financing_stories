...
def export_data_downloadable(preprocessed_df, medians_df) -> None:
    ...
    (pd.concat([
     ...
     .assign(indicator_name="Out-of-pocket health expenditure (% of current health expenditure)")
...
def export_chart(preprocessed_df, medians_df) -> None:
    ...
    (pd.concat([
        medians_df.assign(entity_name=lambda d: d.income_level + " (median)").assign(annotate_color=True),
        preprocessed_df.rename(columns={"country_name": "entity_name"}).assign(annotate_gray=True),
    ], ignore_index=True)

     .dropna(subset=["income_level"])
     .loc[lambda d: d.income_level != "Not classified"]

     .pivot(index=["year", "income_level", "annotate_color", "annotate_gray"], columns="entity_name", values="value")
     .loc[:, lambda d: ["Low income (median)", "Lower middle income (median)", "Upper middle income (median)",
                        "High income (median)"]
                       + [c for c in d.columns if c not in ["Low income (median)", "Lower middle income (median)",
                                                            "Upper middle income (median)", "High income (median)"]]]

     .reset_index()

     # sort income level starting with Low income and ending with High income
     .assign(income_level=lambda d: pd.Categorical(d["income_level"],
