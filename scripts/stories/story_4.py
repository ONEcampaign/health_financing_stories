...
def loess_smooth(df, endog, exog, frac=0.5) -> pd.Series:
    ...
    s = pd.Series(
        sm.nonparametric.lowess(endog=endog, exog=exog, frac=frac, return_sorted=False),
        index=df.index,
    )
    ...
    return (
        GHED_DATA.loc[
            lambda d: d.indicator_code.isin(
                list(chart_1_indicators.values()) + [gdp_var]
            )
        ]
        .dropna(subset=["value"])
        ...
        .groupby(["country_name", "iso3_code"])
        .tail(1)
        .sort_values(
            by=gdp_var
        )  # sort by gdp per capita to have a meaningful loess smoothing
        .reset_index(drop=True)
        #       apply loess smoothing
        .assign(
            hc1_che_loess=lambda d: loess_smooth(
                d, endog=d[chart_1_indicators["hc1"]], exog=d[gdp_var]
            ),
            hc6_che_loess=lambda d: loess_smooth(
                d, endog=d[chart_1_indicators["hc6"]], exog=d[gdp_var]
            ),
        )
...
def chart1():
    ...
    (
        df.pivot(
            index=["country_name", "iso3_code", gdp_var, "year"],
            columns="indicator_code",
            values="value",
        )
        .reset_index()
    ...
    (
        df.assign(
            connet=lambda d: d.indicator_code.map(
                {"hc1_che_loess": "hc1_loess", "hc6_che_loess": "hc6_loess"}
            )
        )
        # color column where indicator code starts with hc1 or hc6
        .assign(
            color=lambda d: np.where(
                d.indicator_code.str.startswith("hc1"), "hc1", "hc6"
            )
        )
...
def prepare_chart2_data():
    ...
    return (
        GHED_DATA.loc[
            lambda d: d.indicator_code.isin(
                list(chart_2_indicators.values()) + [gdp_var]
            )
        ]
        .dropna(subset=["value"])
        .loc[lambda d: (d.year >= 2018) & (d.year < 2023)]
        ...
        #       apply loess smoothing
        .assign(
            hc6_gghed_loess=lambda d: loess_smooth(
                d, endog=d[chart_2_indicators["gghed"]], exog=d[gdp_var]
            ),
            hc6_ext_loess=lambda d: loess_smooth(
                d, endog=d[chart_2_indicators["ext"]], exog=d[gdp_var]
            ),
        )
...
def chart2():
    ...
    (
        df.assign(
            connet=lambda d: d.indicator_code.map(
                {"hc6_gghed_loess": "hc6_gghed_loess", "hc6_ext_loess": "hc6_ext_loess"}
            )
        )
        .assign(
            color=lambda d: np.where(
                d.indicator_code.str.startswith("hc6_ext"), "ext", "gghed"
            )
        )
        .assign(size=lambda d: np.where(d.indicator_code.str.endswith("loess"), 0, 1))
        .assign(popup=lambda d: np.where(d.connet.isnull(), True, None))
